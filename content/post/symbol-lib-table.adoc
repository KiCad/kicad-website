...
3. Symbol naming now follows the same convention as footprint naming.  The
   ...
   issues with broken symbol links.  Please fix you symbol names that have
   these characters.  It is also a good idea to no use any characters that
   are not valid file characters on both Posix and Windows file systems.
   When the new symbol library file format is implemented, illegal file
   system characters will cause issues.
